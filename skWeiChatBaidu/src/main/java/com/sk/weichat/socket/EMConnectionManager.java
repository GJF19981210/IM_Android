...
public class EMConnectionManager {
    ...
    public EMConnectionManager(Context context) {
        ...
        Log.e("xuan", " new EMConnectionManager: " + mCurrentLoginUserToken + ", " + mCurrentLoginUserId +" ,  socket ip: " + SOCKET_HOST);
    ...
    private class SocketThread extends Thread {
        ...
        private void notifyConnect(int which, int authState) {
            Log.e("zq", "which：" + which + "  authState:" + authState);
        ...
        private void decodeSocket() throws IOException {
            ...
            if (count == -1) {
                ...
                while (dataBuffer.hasRemaining()) {
                    ...
                    Log.e("perryn", "数据接收：" + JSON.toJSONString(packet));
                    if (packet == null) {
        ...
        private void decodePacket(TcpPacket packet) {
            ...
            Log.e(TAG, "decode: TCP解码成功 command = " + packet.getCommand());
            /**
             * 登录结果返回 || 登录冲突
             */
            if (packet.getCommand() == Command.COMMAND_AUTH_RESP || packet.getCommand() == Command.COMMAND_LOGIN_CONFLICT_RESP) {
                if (packet.getCommand() == Command.COMMAND_AUTH_RESP) {
                    mLoginIng = false;
                    AuthRespMessage auth = ProBufUtils.decoderMessageBody(packet.getBytes(),
                            MessageProBuf.AuthRespMessageProBuf.getDescriptor(), AuthRespMessage.class);
                    Log.e(TAG, JSON.toJSONString(auth));
